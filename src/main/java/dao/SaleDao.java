...
public class SaleDao implements DaoInterface<Sale> {
    ...
    public boolean create(Sale sale) throws SQLException {
        ...
        Receiving receiving = receivingDao.read(sale.getProductId());
        if (receiving != null) {
            RunningBalanceProduct runningBalanceProduct = runningBalanceProductDao.getRunningBalanceProduct(sale.getProductId());
            if (runningBalanceProduct != null) {
                if (runningBalanceProduct.getRunningBalance() > sale.getQuantity()) {
                    ...
                    if (DBHandler.getInstance().executeAction(query, params, dataTypes) && runningBalanceProductDao.updateRunningBalanceProduct(runningBalanceProduct)) {
