...
public class ReceivingDao implements DaoInterface<Receiving>{
    private RunningBalanceProductDao runningBalanceProductDao = new RunningBalanceProductDao();
    ...
    public boolean create(Receiving receiving) throws SQLException {
        ...
        try {
            if(runningBalanceProductDao.isProductHavingRunningBalance(receiving.getProductId())){
                runningBalanceProduct = runningBalanceProductDao.getRunningBalanceProduct(receiving.getProductId());
                runningBalanceProduct.setRunningBalance(runningBalanceProduct.getRunningBalance()+receiving.getQuantity());
                if(runningBalanceProductDao.updateRunningBalanceProduct(runningBalanceProduct) &&DBHandler.getInstance().executeAction(query,params,dataTypes)){
                ...
                if(runningBalanceProductDao.create(runningBalanceProduct) && DBHandler.getInstance().executeAction(query,params,dataTypes)){
                    DBHandler.getInstance().commitChanges();
                    return true;
                }
