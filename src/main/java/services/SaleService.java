...
public class SaleService implements ServiceInterface<Sale> {
    ReceivingService receivingDao = new ReceivingService();
    ...
    public boolean create(Sale sale) throws SQLException {
        ...
        if (receiving != null) {
            RunningBalanceProduct runningBalanceProduct = runningBalanceProductService.getRunningBalanceProduct(sale.getProductId());
            if (runningBalanceProduct != null) {
                if (runningBalanceProduct.getRunningBalance() > sale.getQuantity()) {
                    ...
                    if (DBHandler.getInstance().executeAction(query, params, dataTypes) && runningBalanceProductService.updateRunningBalanceProduct(runningBalanceProduct)) {
